...
1. To get users new to Hugging Face's `transformers` library familiar with the three main abstractions:
  ...
  - Models
  - Pipelines
2. To give users familiar with `transformers` a whistle-stop tour of the syntax they'll neeed to get started with each abstraction.
...
```{r}
  ...
  tokenizer <- hf_load_tokenizer(model_id, use_fast = FALSE)
```
...
Alternatively, you could use the argument `use_fast = FALSE` when instantiating the tokenizer. It's worth noting that use_fast = TRUE will use the Rust programming language to perform the tokenization. 

Or you can use the `hf_stop_token_spam() which executes the code from the code chunk above.
